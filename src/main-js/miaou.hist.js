...
miaou(function(hist, gui, locals, md, time, ws){
	...
	function moveSelect(d){
		let i, $s = $('#search-results .message.selected');
		if ($s.length) {
		...
		let $selected = $('#search-results .message').eq(i).addClass('selected');
		if ($selected.length) {
			md.focusMessage(+$selected.attr('mid'));
			let	mtop = $selected.offset().top,
	...
	hist.showHist = function(res){
		...
		let sum = 0;
		function day(d, n, sn){
			let	date = new Date(d*24*60*60*1000),
			...
			let $bar = $('<div/>').addClass('bar').css('width', transform(n)*80/maxtn + '%');
			let $day = $('<div/>').addClass('day').append($bar)
	...
	hist.showPage = function(){
		if (!visible) return;
		let sh = gui.$messageScroller.height();
		let $messages = $('#messages .message').filter(function(){
			let y = $(this).offset().top;
			return y<sh && y+$(this).height()>0;
		});
		if (!$messages.length) return;
		let	fd = Math.floor($messages.first().dat('message').created / (24*60*60)),
			ld = Math.floor($messages.last().dat('message').created / (24*60*60));
		$('#hist .day').each(function(){
			let $this = $(this), d = +$(this).attr('d');
	...
	$('#hist').on('click', '.day', function(){
		...
		if (currentResult?.count) {
			let page = Math.floor((currentResult.count-$this.attr("sum"))/currentSearch.pageSize);
	...
	function buildSearchOptions(){
		let options = {pattern: $('#search-input').val().trim()};
