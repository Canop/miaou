(function(){
	...
	function Panel(m, g, s){
		...
		this.s = s; // snap thing
		this.u = -1;
		g.players.forEach(function(p,i){ console.log(p.id, me.id); if (p.id===me.id) this.u=i }, this);
	}
	...
	miaou.games.Tribo = {
		render: function($c, m, g){
			var id = 'ludo_'+m.id;
			$c.empty().css('background', Snap.hsb(.5, .4, .5)).closest('.message').removeClass('edited');
